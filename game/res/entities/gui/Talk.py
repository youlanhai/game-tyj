...
class ChoiceItem(object) :
	...
	#选项是否是确定
	def isOK(self) :
		return self.next == 1001
		
	#选项是否是其他
	def isCancel(self) :
		return self.next == 1002
...
class TalkContent(object) :
	...
	def parse(self, next) :
		...
		self.content = d[1]
		self.choices = []
		for i in xrange(2, len(d)) : #从第2项开始，是后续选择项。
...
class Talk(GUI.Form) :
	...
	def onOK(self) :
		...
		if item.isOK() : #是接受任务选项
			...
			self.talkContent.selected(item.next)
			self._parseNext()
