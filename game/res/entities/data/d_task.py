...
def isExist(no): return sheet.has_key(no)
...
def isExistNpc(no): return sheet_npc.has_key(no)
...
def getNpcSubmitTasks(no) :
	tasks = []
	for k, v in sheet_submit.iteritems() :
		if v["npc"] == no :
			tasks.append(k)
	return tasks
...
def getMapNpc(no) :
	npc = []
	for k, v in sheet_npc.iteritems() :
		if no == v["position"][0] :
			npc.append(k)
	return npc
...
from dd_task import sheet as sheet


#npc，交任务npc
#talk，交任务对白。
from dd_task_submit import sheet as sheet_submit


#position，位置(地图编号，坐标x,y,z)
from dd_task_npc import sheet as sheet_npc

from dd_task_trigger import sheet as sheet_trigger


task_keys = frozenset( sheet.keys() )