...
namespace Physics
{
    ...
    bool BspNode::collision(Physics::CollisionPrevent & ci, const D3DXMATRIX & world) const
    {
    ...
    bool BspNode::collisionThis(Physics::CollisionPrevent & ci, const D3DXMATRIX & world) const
    {
        bool collied = false;
        for (Physics::TriangleSet::const_iterator it = m_triangleSet.begin();
            it != m_triangleSet.end(); ++it)
    ...
    bool BspTree::collision(Physics::CollisionPrevent & ci, const D3DXMATRIX & world) const
    {
        assert(m_root);
#if 0
    ...
    void BspTree::drawDebug(IDirect3DDevice9 *pDevice, const D3DXMATRIX * pWorld/*=NULL*/) const
    {
        if (!pWorld)
        {
            D3DXMATRIX world;
    ...
    BspTreePtr loadBspTree(const Lazy::tstring & bspName)
    {
        ...
        XWRITE_LOGA("----load bsp tree[%s]....", bspName.c_str());

        BspTreePtr tree = new BspTree();
        if(!tree->load(bspName))
        {
            XWRITE_LOGA("加bsp数据[%s]失败！", bspName.c_str());
            tree = NULL;
        }
        g_bspTree2Cache[bspName] = tree;

        XWRITE_LOGA("----load bsp tree[%s] finish.", bspName.c_str());
