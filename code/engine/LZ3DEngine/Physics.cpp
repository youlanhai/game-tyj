...
const float ModelWidth  = 20.0f;
const float ModelHeight = 50.0f;
const float ModelDepth  = 20.0f;
...
void iPhysics::updatePlayer(float fElapse)
{
    ...
    if (m_isJumping)
    {
        ...
        if (upTime > 0.0f)
        {
            ...
            float correct = 10.0f;//降低起点，防止钻入模型。
            start.y -= correct;
            ...
            {
                float distance = max(0, cp.m_distance - correct);
...
void iPhysics::updateHeight(float elapse)
{
    ...
    if (end.y < start.y)
    {
        float correct = 10.0f;//抬高起点，防止钻入模型。
        start.y += correct;
...
void iPhysics::updateAutoMove(float elapse)
{
    ...
    if (m_bMoveToEntity)
    {
        ...
        float inRange = getMaxDistToNpc() - 100;
