...
cSnow::cSnow(int maxAmount, LPCSTR texture, I3DObject* source)
...
void cSnow::setRange(D3DXVECTOR3 min_, D3DXVECTOR3 max_)
{
    m_rangeMin = min_;
    m_rangeMax = max_;
}
...
void cSnow::updateParticle(Attribute *pAttr, float fElapse) 
{
    ...
    if (getMap() != NULL)
    {
        if (pAttr->position.y < getMap()->getHeight( pAttr->position.x, pAttr->position.z))
        {
            resetParticle(pAttr);
        }
    }
