...
//////////////////////////////////////////////////////////////////////////
cSound::cSound(std::string name, LPDSBuffer buffer)
...
/*设置频率。*/
bool cSound::setFrequncy(long fre)
{
    return false;
}

/*设置平衡。*/
bool cSound::setPan(long lpan)
{
    return false;
}

/*设置音量。*/
bool cSound::setVolum(long volume)
{
    return false;
}
...
{
    cSound *pSound = findUnuse(szName);
    if (pSound)
    {
        return pSound;
    }
    ...
    return addToCache(new cSound(szName, buffer));
}
...
cSound* cSoundMgr::addToCache(cSound *sound)
{
    updateCache();
...
cSound* cSoundMgr::findUnuse(std::string name)
{
        ...
        ++it)
    {
        if ((*it)->getName() == name)
        {
            ...
            {
                Lazy::debugMessageA("sound : find one unuse sound [%s] from cache.", name.c_str());
                return it->get();
            }
        }
    }
...
bool cSoundMgr::isSoundUnuse(cSound *sound)
{
