...
bool cMusicPlayer::loadMusic(LPCSTR szName)
{
    if (!m_pDevice)
    {
...
void cMusicPlayer::update(float elapse)
{
...
#if 0
void cMusicPlayer::render(HDC hDC)
{
...
//设置重复播放
void cMusicPlayer::setRepeat(bool repeat)
{ 
    SafeCheck(m_pStream, RT_VOID); 
    m_pStream->setRepeat(repeat); 
}
bool cMusicPlayer::getRepeat(void)
{ 
    SafeCheck(m_pStream, false); 
...
float cMusicPlayer::getVolume(void)
{ 
...
//设置音效平衡
void cMusicPlayer::setPan(float pan)
{
    SafeCheck(m_pStream, RT_VOID); 
    m_pStream->setPan(pan); 
}

float cMusicPlayer::getPan(void)
{ 
    SafeCheck(m_pStream, 0.0f); 
    return m_pStream->getPan(); 
}

void cMusicPlayer::setPitchShift(float shift)
{ 
    SafeCheck(m_pStream, RT_VOID); 
    m_pStream->setPitchShift(shift); 
}

float cMusicPlayer::getPitchShift(void)
{ 
    SafeCheck(m_pStream, 0.0f); 
    return m_pStream->getPitchShift(); 
}

bool cMusicPlayer::isSeekable(void)
{ 
    SafeCheck(m_pStream, false); 
...
//获得音频长度。单位为帧。
int cMusicPlayer::getLength(void)
{ 
...
//设置播放位置。单位为帧。
void cMusicPlayer::setPosition(int position)
{ 
    SafeCheck(m_pStream, RT_VOID); 
    m_pStream->setPosition(position);
}

int cMusicPlayer::getPosition(void)
{ 
    SafeCheck(m_pStream, 0); 
