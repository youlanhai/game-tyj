...
void cTextTexture::init(LPCSTR text, LPCSTR font, DWORD color, int width, int height)
{
    m_pTexture = NULL;
    m_text = text;
    m_font = font;
...
void cTextTexture::setText(LPCSTR text)
{
    m_text = text;
...
cTextTexture* cTextTextureFactory::createObj(LPCSTR text, LPCSTR font, DWORD color, int width, int height)
{
...
void TextTextureEx::setText(const std::string & text)
{
    if (m_text == text)
    {
        return;
...
void TextTextureEx::setFont(const std::string & font)
{
    if (m_font == font)
    {
        return;
...
void TextTextureEx::loadTexture()
{
    ...
    Lazy::tstring fontName = Lazy::charToWChar(m_font);
    Lazy::FontPtr font = Lazy::FontMgr::instance()->getFont(fontName);
    if (!font)
    {
    ...
    std::wstring text = Lazy::charToWChar(m_text);
    font->getTextSize(m_width, m_height, text);
    ...
    if (!m_pTexture)
    {
        ...
        debugMessage(_T("create topboard texture (w=%d, h=%d) : %s"), texWidth, texHeight, text.c_str());
    }

    clearTextrue(m_pTexture, 0, 0);
    font->fillTextToTexture(m_pTexture, 0, 0, text, m_color);

    static bool save = true;
    if (m_pTexture && save)
    {
        save = false;
        D3DXSaveTextureToFile(_T("font/tp.png"), D3DXIFF_PNG, m_pTexture, NULL);
    }
}