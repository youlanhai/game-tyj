...
public:
    ...
    ///朝向某点
    void faceTo(const Physics::Vector3 & dest);

    ///朝向某个方向
    void faceToDir(const Physics::Vector3 & dir);

    ///移动到某点
    void moveTo(const Physics::Vector3 & pos);

    ///移动到entity
    void moveToEntity(I3DObject* taget);

    ///寻路
    bool searchPathEx(Physics::WpPtrArray & way, const Physics::Vector3 & dest);

    ///寻路
    ///TODO: 使用异步方式计算寻路。
    bool searchPath(const Physics::Vector3 & dest);
    ...
    ///是否是玩家的物理
    virtual bool isPlayers(){ return false; }
...
protected:
    ...
    Physics::Vector3     m_nextPos;      //< 下一个目标点
    Physics::Vector3     m_prevPos;      //< 前一个点
