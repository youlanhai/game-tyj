...
//        wp->yMax = yMax;
    ...
    bool ChunkNodeMgr::doLoadConfig()
    {
        ...
        cMap* pMap = cMap::instance();
        if(!pMap || !pMap->isUserfull())
        {
            throw(std::runtime_error("Current map is invalid!"));
        }

        const std::string & mapName = pMap->getMapName();

        if(!mapNameToWpPath(m_wayPath, Lazy::charToWChar(mapName)))
        {
        ...
        //构造寻路数据
        int id = 0;
        for(cMap::NodeIterator it = cMap::instance()->begin();
            it != cMap::instance()->end(); ++it)
        {
    ...
    bool ChunkNodeMgr::doGenerate()
    {
        ...
        std::queue<MapNodePtr> nodeQueue;
        nodeQueue.push(cMap::instance()->getNode(m_rows/2, m_cols/2));
        ...
        const std::string & mapName = cMap::instance()->getMapName();
        ...
        while(!nodeQueue.empty())
        {
            ...
            for(int i=0; i<4; ++i)
            {
                ...
                {
                    nodeQueue.push(cMap::instance()->getNode(dr, dc));
                }
            }
            ...
            std::string caption;
            Lazy::formatStringA(caption, "Navigation(process '%s' %d%%)", mapName.c_str(), percent);
            getApp()->setCaption(caption);

            //if(testN == 1) break;
        }

        getApp()->setCaption("Navigation");
        return result;
    }
